...
import { Layout, OrderSummary } from '@/components/common';
import type { OrderSummaryItem } from '@/components/common';
import { useCart } from '@/contexts/CartContext';
import { useFormField } from '@/hooks/useFormField';
import { validateCustomerFields } from '@/lib/utils/form-validators';
...
export default function PickupCheckoutPage() {
  ...
  const { form, setForm, fieldErrors, setFieldErrors, handleChange } = useFormField({
    name: '',
    email: '',
    phone: '',
    paymentMethod: 'STRIPE' as string,
    notes: '',
  });
  ...
  const validateForm = (): boolean => {
    const errors = validateCustomerFields(form, (key) => t(`validation.${key}`));
    ...
    return (
      <Layout>
        <Container maxWidth="lg" sx={{ py: 8, textAlign: 'center' }}>
          ...
          <Button component={Link} href="/pickup" variant="contained" startIcon={<ArrowBackIcon />}>
            {t('backToPickupMenu')}
          </Button>
        </Container>
      </Layout>
  ...
  const summaryItems: OrderSummaryItem[] = items.map((item) => ({
    ...
    qty: item.qty,
    totalPrice: item.product.price_yen * item.qty,
  }));

  return (
    <Layout>
      <Container maxWidth="lg" sx={{ py: 4 }}>
        <Button component={Link} href="/pickup" startIcon={<ArrowBackIcon />} sx={{ mb: 3 }}>
          {t('backToMenu')}
        </Button>
              ...
              {activeStep === 0 && (
                <>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid size={12}>
                      <TextField label={t('notes')} fullWidth multiline rows={3} value={form.notes} onChange={handleChange('notes')} placeholder={t('notesPlaceholder')} />
                    </Grid>
                  </Grid>
              ...
              {activeStep === 1 && (
                <>
                    ...
                    >
                      ...
                      >
                        <FormControlLabel
                          ...
                          label={
                            <Box>
                              <Typography sx={{ fontWeight: 600 }}>{t('stripePayment')}</Typography>
                              <Typography variant="body2" color="text.secondary">{t('stripeDescription')}</Typography>
                            </Box>
                          }
                        />
                      </Paper>
                        ...
                        <FormControlLabel
                          ...
                          label={
                            <Box>
                              ...
                              <Typography variant="body2" color="text.secondary">{t('cashDescription')}</Typography>
                            </Box>
                          }
                        />
                      </Paper>
                  ...
                  {form.paymentMethod === 'STRIPE' && (
                    <Alert severity="info" sx={{ mb: 3 }}>{t('stripeNotice')}</Alert>
                  )}

                  {form.paymentMethod === 'PAY_AT_PICKUP' && (
                    <Alert severity="info" sx={{ mb: 3 }}>{t('cashNotice')}</Alert>
                  )}
