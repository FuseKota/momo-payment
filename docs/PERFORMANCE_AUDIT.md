...
| `src/app/[locale]/shop/page.tsx` | async 化・サーバー側取得・`revalidate=300` |
| `src/app/[locale]/shop/ShopClient.tsx` | `initialProducts` prop 受け取り、`useEffect` フェッチ撤去 |
| `src/app/[locale]/shop/[slug]/page.tsx` | サーバー側で商品取得し props 供給・`revalidate=300` |
...
- `npm run build`: 成功。`/[locale]/shop` `/[locale]/shop/[slug]` が **SSG/ISR（revalidate 300s）** としてプリレンダリング。
...
### 本番デプロイ後の実測（2026-06-02）

デプロイ後に本番（Desktop・スロットリングなし）で再計測。

| ページ | 指標 | Before | After（ウォーム） | 備考 |
|---|---|---|---|---|
| shop | LCP | 1,456 ms | **1,244 ms** | Load delay 1,310→**69 ms** |
| shop | CLS | 0.00 | 0.00 | |
| 商品詳細 | LCP | 1,522 ms | **1,251 ms** | Load delay 1,242→**69 ms** |
| 商品詳細 | CLS | **0.46** | **0.00** | スピナー撤去で解消 |

- **LCP 画像の発見遅延（Load delay）が 1.3 秒 → 69 ms に激減**。preload + 初期 HTML 出力が機能。
- **商品詳細の CLS 0.46 → 0.00**。
- フォント検証: H1 等の計算済み `font-family` が `"Noto Sans JP", ...` を先頭に解決（自己ホスト適用）。`fonts.gstatic.com` / `fonts.googleapis.com` への外部リクエストは消滅。next/image は WebP で配信（Netlify Image CDN）。

#### 注意点・残課題

1. **デプロイ直後のコールドキャッシュは遅い**: 初回は LCP 9 秒超を観測（Netlify Image CDN・エッジ・ISR が全て空）。`/_next/image` は初回生成が遅く 2 回目以降キャッシュされる。**デプロイ直後の数値は steady-state ではない**。
2. **TTFB のばらつき（~900 ms ウォーム / ~2,400 ms 再生成時）**: 当初 `revalidate = 60` のため低トラフィック時にページ再生成が走り TTFB が跳ねていた。商品カタログは更新頻度が低いため **`revalidate = 300`（5 分）へ調整済み**。エッジキャッシュ滞留が伸び TTFB スパイクを抑制（在庫/価格の反映は最大 5 分遅れるトレードオフ）。
3. **フォント数（~33 woff2）**: CJK サブセット分割のため数は多いが、自己ホスト・`display: swap`・`preload: false`・非レンダーブロッキングで LCP/CLS への影響なし（初回のみの帯域）。
