...
2. **TTFB のばらつき（~900 ms ウォーム / ~2,400 ms 再生成時）**: `revalidate = 60` のため低トラフィック時にページ再生成が走り TTFB が跳ねる。商品カタログは更新頻度が低いため、`revalidate` を長め（例: 300〜1800 秒）にするとエッジキャッシュ滞留が伸び TTFB スパイクを抑えられる（在庫/価格反映は遅くなるトレードオフ）。
