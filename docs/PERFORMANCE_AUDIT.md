...
> **別タスク（本対応外）**: フォントを意図通り適用したい場合は、テーマ/CSS の `'Noto Sans JP'` を `var(--font-noto-sans-jp)` に置き換え、`'Noto Serif JP'`（現在未ロード）を next/font で読み込む必要がある。ただしこれは全体の描画フォントが変わるため、デザイン確認を伴う独立タスクとして扱う。
