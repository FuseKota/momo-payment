...
| 購入体験 | ① 店頭受け取り（Stripe 事前決済 or 店頭払い）／② 配送 EC（Stripe 決済必須） |
...
- **入金確認（店頭払い）**: 店頭で入金を受けたら注文詳細の「入金確認（mark-paid）」で `PAID` に更新。
- **発送登録（配送）**: 「発送登録（ship）」で配送業者・追跡番号を入力 → `SHIPPED` に更新 → 顧客へ発送通知メールが自動送信。

#### ステータスの見方

| ステータス | 意味 | 次のアクション |
|-----------|------|--------------|
| `RESERVED` | 店頭払いで予約済（未入金） | 店頭で入金 → 入金確認 |
| `PENDING_PAYMENT` | Stripe 決済待ち | 顧客の決済完了を待つ（自動で `PAID`） |
| `PAID` | 入金済 | 受取/梱包の準備 |
| `PACKING` | 梱包中（配送） | 発送準備 |
| `SHIPPED` | 発送済（配送） | 配送完了を待つ |
| `FULFILLED` | 完了 | — |
...
| 業務 | 方法 |
|-----|------|
| 返金 | Stripe ダッシュボードから該当決済を返金。返金後は注文ステータスを手動で整合させる |
| 入金確認 | 配送は自動（Webhook）。店頭払いは管理画面で手動「入金確認」 |
...
> 返金は現状アプリ側に自動連動しません。返金時は Stripe 側操作後、必要に応じて注文メモ等で記録してください。
...
| `PENDING_PAYMENT` 自動失効 | 未実装。決済中断注文は手動確認 |
| 返金の自動連動 | なし。Stripe 側操作後に手動整合 |
| 温度帯混在 | 冷凍とグッズの同一注文は不可（仕様） |
| 旧テーブル名 | `square_webhook_events` は旧 Square 仕様の名残（現行は Stripe） |
| ドキュメント | `docs/REQUIREMENTS.md` は決済が「Square」表記のまま（実装は Stripe）。改訂推奨 |
