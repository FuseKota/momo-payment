...
| 3 | 店頭受け取り注文 | 注文フォーム、店頭払い/Stripe 決済選択 | 🛒 |
| 4 | 配送EC注文 | 住所入力、配送日時指定、Stripe 決済 | 🛒 |
| 5 | 決済 | Stripe Checkout、Webhook 入金確定 | 🛒⚙️ |
| 6 | 注文完了/照会 | 完了画面、注文番号照会 | 🛒 |
| 7 | 顧客アカウント | 登録・ログイン・ログアウト | 🛒 |
| 8 | マイページ | 注文履歴・注文詳細・住所管理 | 🛒 |
| 9 | ニュース | 一覧・詳細 | 🛒 |
| 10 | 台湾夜市 | 特設ページ・開催カレンダー | 🛒 |
| 11 | 多言語 | 日本語・繁体字中文切替 | 🛒⚙️ |
| 12 | 法定ページ | 特商法・プライバシーポリシー | 🛒 |
| 13 | 商品管理 | CRUD・画像・バリエーション・並び替え | 🛠 |
| 14 | 注文管理 | 一覧・詳細・入金確認・発送登録 | 🛠 |
| 15 | ニュース管理 | CRUD・公開制御 | 🛠 |
| 16 | カレンダー管理 | 夜市カレンダー編集 | 🛠 |
| 17 | メール通知 | 注文確認・発送通知 | ⚙️ |
| 18 | 認証・認可 | 顧客/管理者認証、RLS | ⚙️ |
| 19 | セキュリティ | レート制限・CSRF・CSP・PII マスク・Webhook 署名 | ⚙️ |
| 20 | 運用補助 | ヘルスチェック・郵便番号検索・シード/管理者作成 | ⚙️ |
...
| 機能 | 説明 | API |
|-----|------|-----|
| 注文フォーム | 商品選択・顧客情報（氏名/電話/メール）入力 | `/pickup` |
| 支払い方法選択 | 店頭払い（`PAY_AT_PICKUP`）or Stripe 事前決済 | `/checkout/pickup` |
| 注文作成 | 注文・明細を作成し注文番号を発行 | `POST /api/orders/pickup` |
...
| 入金確認（店頭払い） | `RESERVED → PAID` | `POST /api/admin/orders/[id]/mark-paid` |
...
| POST | `/api/orders/pickup` | 店頭受け取り注文作成 |
...
| POST | `/api/admin/orders/[id]/mark-paid` | 入金確認 |
